...
export class EnhancedStreamManager {
    ...
    async initSession() {
        try {
            // Récupérer l'ID de session du serveur
            const response = await fetch(`${this.serverUrl}/api/session`);
            const sessionInfo = await response.json();
            
            this.sessionId = sessionInfo.sessionId || `session_${Date.now()}`;
            ...
            console.warn('Failed to get server session, using local ID:', error);
            this.sessionId = `local_session_${Date.now()}`;
            this.sessionStartTime = Date.now();
